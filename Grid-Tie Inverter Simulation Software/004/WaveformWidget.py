...
class WaveformWidget(QWidget):
    ...
    def setup_three_phase(self):
        ...
        for phase in ['Phase A', 'Phase B', 'Phase C']:
            ...
            plot_widget.setTitle(title, color='#555555', size='14pt')
            plot_widget.setLabel('left', 'Amplitude (V/A)', color='#FFFFFF', size='12pt')
            plot_widget.setLabel('bottom', 'Time (s)', color='#FFFFFF', size='12pt')
    ...
    def get_plot_title(self, phase=None):
        if self.multilevel_topology == "None" and self.mppt == "None" and self.control == "PI":
            return f"{phase} Waveforms" if phase else "Inverter Output Waveforms"
        return f"{phase} Waveforms ({self.multilevel_topology}, {self.pwm_technique}, {self.design}, {self.mppt}, {self.control})" if phase else f"Inverter Output Waveforms ({self.multilevel_topology}, {self.pwm_technique}, {self.design}, {self.mppt}, {self.control})"
