...
from AdaptiveKontrollstrategien import AdaptiveControlWindow  # New import
import numpy as np
import csv
import os
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.acs_window = None  # Initialize ACS window
        ...
        self.control_panel.launch_grid.connect(self.launch_grid_window)
        self.control_panel.launch_acs.connect(self.launch_acs_window)  # Connect ACS signal
    ...
    def update_waveforms(self):
        grid_voltage = self.grid_window.generate_grid_voltage() if self.grid_window is not None else np.zeros(self.simulation.samples)
    ...
    def launch_acs_window(self):
        if self.acs_window is None or not self.acs_window.isVisible():
            self.acs_window = AdaptiveControlWindow(self.simulation)
            self.acs_window.show()
    ...
    def export_all_data(self):
        ...
        if self.tds_window is not None and self.tds_window.simulation_data.get('time') is not None:
            data['tds'] = self.tds_window.simulation_data

        if self.fsa_window is not None and self.fsa_window.analysis_data.get('freqs') is not None:
...
if __name__ == '__main__':
    ...
    app.setAttribute(Qt.AA_EnableHighDpiScaling)  # Support high-DPI displays
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())