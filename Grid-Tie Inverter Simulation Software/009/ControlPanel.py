from PyQt5.QtWidgets import QWidget, QVBoxLayout, QGroupBox, QSlider, QDoubleSpinBox, QLabel, QPushButton, QHBoxLayout, QComboBox, QCheckBox, QFileDialog
...
class ControlPanel(QWidget):
    ...
    launch_tds = pyqtSignal()
    launch_fsa = pyqtSignal()
    launch_grid = pyqtSignal()
    launch_acs = pyqtSignal()
    export_all_data = pyqtSignal()
    ...
    def init_ui(self):
        ...
        self.dc_source_combo.currentTextChanged.connect(self.update_dc_source_ui)
        self.dc_source_combo.currentTextChanged.connect(self.emit_dc_source)

        # DC Source Parameters Group
        dc_source_group = QGroupBox("DC Source Parameters")
        dc_source_layout = QVBoxLayout()
        dc_source_layout.setSpacing(10)
        dc_source_group.setLayout(dc_source_layout)
        ...
        dc_source_layout.addWidget(self.dc_voltage_label)
        dc_source_layout.addWidget(self.dc_voltage_spin)
        dc_source_layout.addWidget(self.irradiance_label)
        dc_source_layout.addWidget(self.irradiance_spin)
        dc_source_layout.addWidget(self.temperature_label)
        dc_source_layout.addWidget(self.temperature_spin)
        dc_source_layout.addWidget(self.soc_label)
        dc_source_layout.addWidget(self.soc_spin)
        dc_source_layout.addWidget(self.load_current_label)
        dc_source_layout.addWidget(self.load_current_spin)
        ...
        self.multilevel_combo.currentTextChanged.connect(self.emit_multilevel)  # Fixed typo
        ...
        self.tds_button = QPushButton("TDS")
        self.fsa_button = QPushButton("FSA")
        self.grid_button = QPushButton("Grid")
        self.acs_button = QPushButton("ACS")
        self.export_all_button = QPushButton("Export")

        button_layout.addWidget(self.start_button)
        button_layout.addWidget(self.pause_button)
        button_layout.addWidget(self.reset_button)
        button_layout.addWidget(self.tds_button)
        button_layout.addWidget(self.fsa_button)
        button_layout.addWidget(self.grid_button)
        button_layout.addWidget(self.acs_button)
        ...
        self.fsa_button.clicked.connect(self.launch_fsa.emit)
        self.grid_button.clicked.connect(self.launch_grid.emit)
        self.acs_button.clicked.connect(self.launch_acs.emit)
        self.export_all_button.clicked.connect(self.export_all_data.emit)
        ...
        params_layout.addWidget(dc_source_group)
    ...
    def getStyleSheet(self):
        return """
            ...
            QSlider, QDoubleSpinBox, QComboBox, QCheckBox, QPushButton {
            ...
            QPushButton#tds_button, QPushButton#fsa_button, QPushButton#grid_button, QPushButton#acs_button, QPushButton#export_all_button {
                font-family: 'Consolas';
                font-size: 12pt;
            }
            QLabel {
    ...
    def update_dc_source_ui(self):
        source_type = self.dc_source_combo.currentText()
        self.dc_voltage_label.setVisible(source_type == "Fixed")
        self.dc_voltage_spin.setVisible(source_type == "Fixed")
        self.irradiance_label.setVisible(source_type in ["PV Panel", "Hybrid"])
        self.irradiance_spin.setVisible(source_type in ["PV Panel", "Hybrid"])
        self.temperature_label.setVisible(source_type in ["PV Panel", "Hybrid"])
        self.temperature_spin.setVisible(source_type in ["PV Panel", "Hybrid"])
        self.soc_label.setVisible(source_type in ["Battery", "Hybrid"])
        self.soc_spin.setVisible(source_type in ["Battery", "Hybrid"])
        self.load_current_label.setVisible(source_type in ["Fuel Cell", "Hybrid"])
        self.load_current_spin.setVisible(source_type in ["Fuel Cell", "Hybrid"])
    ...
    def emit_parameters(self):
        params = {
            ...
            'dc_source_type': self.dc_source_combo.currentText()
        }
